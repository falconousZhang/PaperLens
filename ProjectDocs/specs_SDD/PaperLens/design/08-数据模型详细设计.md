...
| storage_key | VARCHAR(1024) | | 页面图片存储路径（预留） |

**索引:**

```sql
CREATE UNIQUE INDEX idx_paper_page_paper_id ON paper_pages(paper_id, page_number);
```

### 2.3 PaperSection (论文章节表)
...
```sql
CREATE UNIQUE INDEX idx_paper_chunk_paper_id ON paper_chunks(paper_id, chunk_index);
```

### 2.5 PaperTable (论文表格表)
...
```sql
CREATE UNIQUE INDEX idx_paper_table_paper_id ON paper_tables(paper_id, page_number, table_index);
```
...
CREATE UNIQUE INDEX idx_review_dimension ON review_results(task_id, dimension);
```

### 2.9 ReviewFinding (审阅发现表)
...
### 2.10 finding_evidence (发现-证据关联表)
...
```sql
CREATE UNIQUE INDEX idx_finding_evidence ON finding_evidence(finding_id, evidence_id);
```

### 2.11 MetricRecord (实验指标记录表)
...
| checkpoint_type | VARCHAR(20) | | FINAL / MAX / MEAN / BEST / LAST / UNKNOWN |
| checkpoint_source | VARCHAR(50) | | EXPLICIT_TEXT / IMPLICIT_CONTEXT / TABLE_HEADER / UNKNOWN |
...
### 2.13 ExperimentResult (实验数据分析结果表)
...
```sql
CREATE UNIQUE INDEX idx_exp_result_file_id ON experiment_results(file_id);
```

### 2.14 ExportReport (导出报告表)
...
| 约束名 | 表 | 约束描述 |
|--------|-----|----------|
| ck_paper_status | Paper | status IN ('UPLOADING', 'PROCESSING', 'PARSED', 'FAILED') |
| ck_task_status | AnalysisTask | status IN ('PENDING', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELLED') |
| ck_task_progress | AnalysisTask | progress BETWEEN 0 AND 100 |
| ck_review_rating | ReviewResult | rating BETWEEN 1 AND 5 |
| ck_finding_confidence | ReviewFinding | confidence BETWEEN 0.0 AND 1.0 |
| ck_finding_verification | ReviewFinding | verification_status IN ('VERIFIED', 'UNVERIFIED', 'PENDING') |
| ck_evidence_type | Evidence | evidence_type IN ('TEXT', 'TABLE', 'FIGURE_CAPTION', 'EQUATION') |
| ck_finding_type | ReviewFinding | finding_type IN ('STRENGTH', 'WEAKNESS', 'SUGGESTION') |
| ck_export_status | ExportReport | status IN ('PENDING', 'GENERATING', 'READY', 'FAILED') |
...
- finding_evidence.finding_id → ReviewFinding.id, finding_evidence.evidence_id → Evidence.id
