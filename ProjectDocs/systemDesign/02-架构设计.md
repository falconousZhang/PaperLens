...
| UI组件库 | Element Plus | - | PC 端组件库 |
| 状态管理 | Pinia | 2.x | Vue3 官方推荐状态管理 |
...
sequenceDiagram
    participant E as ECS 后端
    participant V as FAISS 向量索引
    participant M as ModelArts LLM
    participant D as RDS PostgreSQL

    E->>E: 1. 根据审阅维度构造检索 query
    E->>V: 2. 检索 Top-K 相关 PaperChunk
    V-->>E: 3. 返回相关文本块
    E->>M: 4. Prompt + Evidence 传入 LLM
    M-->>E: 5. 返回审阅结果（含 Evidence ID 标注）
    E->>E: 6. 后处理验证：每条 Finding 必须关联至少一个 Evidence
    E->>D: 7. 写入 ReviewResult + ReviewFinding + Evidence 绑定
