...
2. **OBS Key 构造**：
   - 使用 UUID 作为文件存储名，不使用用户提供的文件名
   - OBS Key 格式：`papers/{user_id}/{uuid}/{safe_filename}`
   - 不允许用户直接指定 OBS Key

3. **文件读取**：
   - 读取 OBS 文件时仅使用数据库中记录的 obs_key
   - 不接受用户输入的文件路径参数
...
| 数据类型 | 保留期限 | 清理方式 |
|---------|---------|---------|
| 论文原文（OBS） | 用户删除时立即清理 | 同步删除 OBS 对象 |
| 解析中间结果（OBS） | 用户删除时立即清理 | 同步删除 OBS 对象 |
| 结构化数据（RDS） | 用户删除时级联删除 | 数据库 CASCADE |
| 向量索引（FAISS） | 用户删除时重建索引 | 异步重建 |
| 导出报告（OBS） | 7 天后自动清理 | 定时任务扫描 |
